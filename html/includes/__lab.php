	...
	public function __construct($f, $tenant) {
		...
		if (!is_file($f)) {
			...
			foreach ($xml -> xpath('//lab/topology/nodes/node') as $node_id => $node) {
                ...
                if (isset($node -> attributes() -> docker_options)) $n['docker_options'] = (string) $node -> attributes() -> docker_options;
				...
				foreach ($this -> getNodes() as $node_id => $node) {
					...
					switch ($node -> getNType()) {
						...
						case 'docker':
                            ...
                            if ( $node -> getDocker_options() != '' ) $d -> addAttribute('docker_options', $node -> getDocker_options());
							break;
						case 'qemu':
