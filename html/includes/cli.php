	...
	switch ($p['type']) {
		...
		case 'bridge':
			...
			if (!isInterface($p['name'])) {
                ...
                error_log(date('M d H:i:s ').'INFO: Add network bridge - Interface does not exist '.$p['name']);
				return addBridge($p);
			} else if (isBridge($p['name'])) {
				// Bridge already present
                error_log(date('M d H:i:s ').'INFO: Add network bridge - bridge present '.$p['name']);
				return 0;
...
function prepareNode($n, $id, $t, $nets) {
	...
	// Creating TAP interfaces only for non docker nodes
			...
			case 'docker':
				...
				if ($rc != 0) {
					...
					if ($n->getDocker_options() == '') {
                        ...
                        $cmd = 'docker -H=tcp://127.0.0.1:4243 create -ti --memory ' . $n->getRam() . 'M --privileged --net=none -p ' . $n->getPort() . ':' . $connPort . ' --name=' . $n->getUuid() . ' -h ' . $n->getName() . ' ' . $n->getImage();
                        error_log(date('M d H:i:s ') . 'INFO: starting ' . $cmd);
                        exec($cmd, $o, $rc);
                    } else {
						// docker options exist
                        $cmd = 'docker -H=tcp://127.0.0.1:4243 create -ti --memory ' . $n->getRam() . 'M --privileged '.$n->getDocker_options().' --net=none -p ' . $n->getPort() . ':' . $connPort . ' --name=' . $n->getUuid() . ' -h ' . $n->getName() . ' ' . $n->getImage();
                        error_log(date('M d H:i:s ') . 'INFO: starting ' . $cmd);
                        exec($cmd, $o, $rc);
					}
					if ($rc != 0) {
                ...
                /* unlinks .lock for docker but the lock file is not created at this point. Why does QEMU function with removal at this point?
...
function start($n, $id, $t, $nets, $scripttimeout) {
    ...
    error_log(date('M d H:i:s ').'INFO:cli.php:preparenode: flags '.$flags.' bin: '.$bin.'.');
	...
	switch ($n -> getNType()) {
		...
		case 'iol':
			...
			break;
		case 'docker':
			//Need to set environmentals in create rather than start
			$cmd = 'docker -H=tcp://127.0.0.1:4243 start '.$n -> getUuid();
			error_log(date('M d H:i:s ').'INFO: starting:cli.php:start: '.$cmd.' '.$n ->getUuid().'.');
			break;
		case 'vpcs':
	...
	if ($rcp == 0 && $n -> getNType() == 'docker') {
		...
		foreach ($n -> getEthernets() as $interface_id => $interface) {
            ...
            $cmd = 'ip link delete vunl'.$t.'_'.$id.'_'.$interface_id;
            error_log(date('M d H:i:s ').'INFO: deleting '.$cmd);
            exec($cmd, $o, $rc);

			// ip link add docker3_4_5 type veth peer name vnet3_4_5
			$cmd = 'ip link add docker'.$t.'_'.$id.'_'.$interface_id.' type veth peer name vunl'.$t.'_'.$id.'_'.$interface_id;
			error_log(date('M d H:i:s ').'INFO: starting '.$cmd);
			exec($cmd, $o, $rc);
			...
			$cmd = 'ip link set dev vunl'.$t.'_'.$id.'_'.$interface_id.' up';
			error_log(date('M d H:i:s ').'INFO: starting '.$cmd);
			exec($cmd, $o, $rc);
			// brctl addif vnet0_1 vnet3_4_5

            $cmd = 'brctl addif vnet'.$t.'_'.$interface -> getNetworkId().' vunl'.$t.'_'.$id.'_'.$interface_id;
            error_log(date('M d H:i:s ').'INFO: starting '.$cmd);
            exec($cmd, $o, $rc);
            //
            ...
            $cmd = 'ip link set netns '.$o[1].' docker'.$t.'_'.$id.'_'.$interface_id.' name eth'.$interface_id.' address '.'50:'.sprintf('%02x', $t).':'.sprintf('%02x', $id / 512).':'.sprintf('%02x', $id % 512).':00:'.sprintf('%02x', $interface_id).' up';
            error_log(date('M d H:i:s ').'INFO: starting '.$cmd);
            exec($cmd, $o, $rc);
