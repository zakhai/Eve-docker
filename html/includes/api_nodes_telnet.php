...
function apiGetLabNodeTemplate($p) {
	...
	if ($p['type'] == "qemu") {
				...
				'list' => Array ( 'virtio-net-pci' => 'virtio-net-pci' ,'e1000' => 'e1000','e1000-82545em' => 'e1000-82545em', '' => 'tpl'.( isset($p['qemu_nic'])?'('.$p['qemu_nic'].')':"(e1000)")));
	...
	if ($p['type'] == 'qemu') {
  	...
  	if ($p['type'] == 'docker') {
                $*output['data']['options']['console'] = Array(
                        ...
                        'type' => 'list',
                        'value' => $p['console'],
                        'list' => Array('telnet' => 'telnet', 'vnc' => 'vnc' , 'rdp' => 'rdp' , 'ssh' => 'ssh')
                );
	}
/*
